...
const Page = ({ params }) => {
  ...
  useEffect(() => {
    const getSingleSubCategoryInformation = async () => {
      try {
        ...
        setSingleSubCategory(data?.result);
        setFormData({
          xsubcat: data?.result?.xsubcat ? data.result.xsubcat : "",
        });
      } catch (error) {
        toast.error("Error fetching sub category information:", error.message);
      }
    };
  ...
  const submitFormData = async () => {
    ...
    try {
      ...
      if (response.ok === true) {
        customRevidateTag("single-sub-category");
        toast.success("The sub category was updated successfully!");
      }
    } catch (error) {
            ...
            <div>
              ...
              >
                <h1 className="font-medium text-deepBlue">
                  {selectedValue ? selectedValue : catName ? catName : "Choose"}
                </h1>
                <MdKeyboardArrowDown />
              </div>
