...
const CreateDistrict = () => {
  ...
  const submitFormData = async () => {
    ...
    try {
      ...
      if (response.ok === true) {
        toast.success("A new district was created successfully!");
        setUpdateStatus(true);
        router.refresh();
        // customRevidateTag("code");
      }
    } catch (error) {
      console.error("Error posting data:", error.message);
      toast.error("Failed to create district. Please try again.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex flex-row gap-4 mt-5">
            {updateStatus ? (
              <>
                <Link href={`/dashboard/districts/edit/${updateId}`}>
